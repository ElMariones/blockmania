...
**Endless** is a separate arcade game from the main menu: place from a three-piece tray, clear rows and columns, build a combo up to x10, and play until no tray piece or already stored Hold piece fits. Each fresh tray has at least one legal option. Drag a selected piece into Hold or press H to store or swap it once per placement; an empty Hold cannot rescue a tray that is already stuck. The right rail lets you choose among 12 block finishes, with two animated finishes to unlock. Endless saves independently, with **Continue Endless** and a local top-ten high-score view that includes run statistics and score graphs.
