...
**Plan items, bosses and menus (2026-09-24, GDD §22):** a campaign Hold slot, a choice of round card before every non-boss round, Mk II bosses from act 2, Heat 0–5 stakes on the Kit screen, Jokers unlocked by achievements, a local Daily run, RUN HISTORY, Overtime milestones, contextual tips, a boss cinematic and edge shader (hazard frame, danger heartbeat, heat haze), six new songs, and a tabbed Pause/Options screen with a RUN page, game speed, shake and flash strength.

Controls: drag a shape onto the board, or click a shape and then click a cell. Right-click or Esc cancels. Keyboard: `1`–`3` select a shape, arrows/WASD move it, Enter/Space place, `R` refresh, `B` shows your bag, `M` toggles sound, Esc pauses (in the shop too: SAVE & MAIN MENU returns to the title). In the Trophy Case, Q/E or PageUp/PageDown turn pages. Options (title or pause) has pages for Game, Audio, Display, Accessibility and Controls (Q/E switch pages); in a run a RUN page shows the round track, the Heat rules and this act's boss. `H` holds a piece.
