...
func test_crossbar_needs_row_and_column() -> void:
	...
	eq(_joker_value(_place(cross, shape(&"single"), Vector2i(7, 7), ["crossbar"]), "crossbar"), 100, "row+col")
...
func test_small_change_and_heavy_hand() -> void:
	eq(_joker_value(_place(EMPTY_ROWS, shape(&"bar3"), Vector2i(0, 0), ["small_change"]), "small_change"), 60, "3 cells x 20")
...
func test_blue_mood_and_color_blind() -> void:
	eq(_joker_value(_place(EMPTY_ROWS, shape(&"single", 0, BMShapes.COLOR_BLUE), Vector2i(0, 0), ["blue_mood"]), "blue_mood"), 1.0, "blue")
...
func test_chain_link_uses_combo() -> void:
	...
	eq(_joker_value(r, "chain_link"), 1.5, "0.5 x combo 3")


func test_wide_awake_hollow_point_pressure_cooker() -> void:
	var two := [".......1", ".......1", ".......1", ".......1", ".......1", ".......1", ".......1", "1111111."]
	eq(_joker_value(_place(two, shape(&"single"), Vector2i(7, 7), ["wide_awake"]), "wide_awake"), 2.0, "2 lines")
	eq(_joker_value(_place(EMPTY_ROWS, shape(&"single"), Vector2i(0, 0), ["hollow_point"]), "hollow_point"), 1.5, "empty board")
	var busy := ["1111111.", "1111111.", "1111111.", "1111111.", "1111111.", "........", "........", "........"]
	# 35 occupied -> 29 empty: no Hollow Point; Pressure Cooker floor(35/4)=8 -> +4 (cap)
	eq(_joker_value(_place(busy, shape(&"single"), Vector2i(0, 7), ["hollow_point"]), "hollow_point"), null, "busy board")
	eq(_joker_value(_place(busy, shape(&"single"), Vector2i(0, 7), ["pressure_cooker"]), "pressure_cooker"), 4.0, "capped at 4")
	eq(_joker_value(_place(ONE_ROW, shape(&"single"), Vector2i(0, 7), ["pressure_cooker"]), "pressure_cooker"), 0.5, "7 occupied -> +0.5")
...
func test_color_cycle_three_distinct() -> void:
	...
	eq(_joker_value(run.place(2, Vector2i(4, 4)), "color_cycle"), 2.0, "three distinct colors")
...
func test_long_game() -> void:
	...
	eq(r.chips, 0, "no cell chips for first three placements")
...
func test_second_look_and_spare_parts() -> void:
	...
	eq(run.credits - credits, 3 + 3 + 1, "credits")
...
func test_stuck_with_refresh_prompts_instead_of_losing() -> void:
	...
	check(BMTrayGenerator.any_fits(run.board, run.tray), "refreshed tray has a legal shape")
