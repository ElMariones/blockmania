...
- [x] **Items rework** (owner, 2026-09-26; GDD §25.2): Punch → Eraser (3 blocks), Coffee Break → Second Tray, Cash Out → Coin Roll, Blueprint → Emergency Brick (retired from every source; held ones still work); Polish +40 Chips × round, Spark +1 Mult × act; live values on the item cards; new Lucky Draw, Mystery Stamp, Double Down (chance items on their own `items` stream) and Phantom Line. Not done: sell or refund unused items, value on the drag preview.
- [x] **Shop repetition** (owner, 2026-09-26): owned Jokers are offered at 1/5 weight (copies still possible) with an OWNED tag on their shop card. The "skip last visit's Jokers" variant was not adopted.
- [x] **Round cards are real bets** (owner, 2026-09-26; GDD §25.5): Double or Nothing ×1.7 +7, Tight Budget −5 +5, Gold Rush +1, Rush Hour +3, Mult Fever ×1.5, Treasure Hunt ×1.35, Scholarship ×1.4.
- [x] **Late Credits** (owner, 2026-09-26; GDD §25.3–25.4): Tuning Fork (Joker levels) and Item Pouch (up to 4 item slots) in act-2+ Workshops; the **Holo shelf** from the shop after round 8 (Negative Film, AGAIN Seal, Master Schematic, Master Tuning, Legend Crate, Hologram; prices rise 50% per purchase; max 3 Negative and 3 AGAIN Jokers). Chrome frame, foil / negative / AGAIN card shader, 12 new portraits.
- [x] **Act bosses ×1.15** (rounds 4 and 8).
- [ ] Rare Tray Hands more often, hidden commons, first-timer shop hints: **declined by the owner** (2026-09-26).
...
- [x] **Color and form Jokers** (owner request, 2026-09-26; GDD §25.7): a common and a rare for each block color (5 new commons beside Blue Mood, 6 rares scaling with that color's pieces in the bag) and each piece form (4 new commons beside Straight Edge, Architect and Square Deal, 7 rares scaling with that form's pieces), with 22 original portraits (`tools/art/cards_palette.py`) and translations. 92 Jokers in all.
- [x] **Board pressure** (owner request: running out of room should be part of the game; GDD §25.6): refilled trays are no longer guaranteed to fit (only a round's first tray, Refresh and Second Tray), the board carries over between the rounds of an act, and 0/2/3/4 rubble stones drop per round by act. Round intro says what is on the board. Simulated: board locks went from ~0% of losses to most losses of loose play, while planners still lose mostly to placements.
- [x] **Targets** ×1.15 / ×1.35 / ×1.55 for rounds 3+ by act (Overtime grows from round 12's); see balance watch.
...
- 2026-09-26: **study follow-up** (carried board, rubble 0/2/3/4, act target scales ×1.15/×1.35/×1.55, act bosses ×1.15, twists retuned, Holo shelf, 22 color/form Jokers). Paired arms (60 seeds): planner 68% (was 83%), expert 75% (85%), careful 25% (30%), casual 0% (3%). Population (400 runs with Overtime): first-timer 0%, casual 21%, engaged 75%, expert 83%, item lover 84%; board-lock losses 21.5% of all losses (all first-timers, was 2.3%). Machine broken 2/400 population, 1/60 `ot_best` (7/60 before Holo price rises and caps). Report: `docs/balance/study_followup_2026-09-26.md`. Watch: competent bots never lock the board; confirm with humans. Chip color commons were dead weight (0–6% wins held) and became +2 Mult after the batch (not re-simulated).
