...
extends RefCounted
## Draws toy blocks from the pixel-art kit (assets/ui/block_*.png, 22x22 art px) with material
## overlays and stamp badges. Every material has a shape cue besides color (streaks, double
## outline, coin, glare, color band); the optional accessibility patterns add a per-color mark.

static var show_patterns := false


static func draw_block(ci: CanvasItem, rect: Rect2, color_id: int, alpha: float = 1.0, material: String = "", tint: Color = Color.WHITE) -> void:
	var a := alpha * (0.62 if material == "glass" else 1.0)
	var mod := Color(tint.r, tint.g, tint.b, a)
	ci.draw_texture_rect(BMStyle.block_tex(color_id), rect, false, mod)
	if material != "":
...
static func draw_shape(ci: CanvasItem, shape: Dictionary, origin: Vector2, cell: float, alpha: float = 1.0, tint: Color = Color.WHITE) -> void:
	var material := String(shape.get("material", ""))
	for c: Vector2i in shape.cells:
		draw_block(ci, Rect2(origin + Vector2(c) * cell, Vector2(cell, cell)), int(shape.color), alpha, material, tint)
