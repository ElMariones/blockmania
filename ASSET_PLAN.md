...
| Steam store description kit: localized banner, section headers, core-loop strip, Joker / finish / achievement panels (all eleven game languages, plus Latin American Spanish and European Portuguese copies), framed screenshots, gameplay and boss clips (MP4 + WEBM, 1280×720), 13 store screenshots 1920×1080 | `tools/store/capture.py` + `tools/store/build_store.py` → `store/steam/` | original, from the game's own captures and art; Japanese and Chinese text drawn with the Fusion Pixel 10 px fonts (OFL-1.1, as in game; 2026-09-26, replaced Microsoft YaHei) | Produced (2026-09-25) |
