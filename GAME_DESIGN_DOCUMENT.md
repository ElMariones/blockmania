...
**Hold:** one shape can be stored in the right-hand Hold well by dragging a selected tray piece there, clicking it with a selected piece, or pressing H. An empty Hold draws a seeded replacement into that tray slot; a filled Hold swaps its piece with the selected slot. Hold may be used once between placements and recharges after a successful placement. An empty Hold may be used while at least one offered tray piece fits; its replacement is guaranteed to fit if that hold action would otherwise leave no fitting tray piece. A swap that would leave no playable tray piece is rejected. Loss occurs immediately when no tray piece fits and there is no already stored fitting Hold piece available to swap. An empty Hold cannot draw a rescue after the tray is stuck. Hold state and cooldown are saved with the game.
