...
- **Language** (Settings > Game, 2026-09-25): SYSTEM (follows the language chosen for the game in Steam, else the OS, else English) or one of eleven languages, each button in its own script: English, Español, Français, Italiano, Deutsch, Nederlands, Polski, Português (BR), 日本語, 简体中文, 繁體中文. The switch applies at once and rebuilds the screens in place; a run, the shop or an Endless game in progress is untouched, and saves never contain translated text. Names (BLOCKMANIA, POPS, Buru Arcade) and the game terms Chips/Mult stay in Latin languages where they read naturally; Japanese and Chinese translate everything. Numbers group thousands with a comma (en, ja, zh) or a narrow space (the other languages).
