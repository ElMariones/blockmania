...
- Round-play update cues: `tools/audio/gen_sfx_features.py` writes 29 more WAVs (tray reels, five Hand fanfares, item targeting and tool hits, the Emergency Brick grab/whoosh/bounce/impact, Feat medal, Patience tick, Loan Shark, Insurance, Warden lock/unlock, Undertaker tomb, Boss Crate), with helpers from `gen_sfx.py`. Run `python tools/audio/gen_sfx_features.py`.
