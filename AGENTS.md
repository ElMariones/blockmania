...
- Endless uses a seeded fair-trio replacement when all three new offers are illegal, one Hold action between placements, and a saved x1/x2/x3/x5/x8/x10 combo ladder that resets after three misses. A stuck tray ends the run unless an already stored fitting Hold piece can be swapped in; an empty Hold never prevents loss. Keep presentation cues derived from the action result.
