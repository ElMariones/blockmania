...
| `game/run/` | `BMRun` (complete run state + every player command, history, replay, `to_dict`/`from_dict`), `BMSaveStore` (local saves/settings/Kit profile) and `BMAchievementStore` (unlocks, seen flags, lifetime data, personal records in `user://achievements.cfg`). `BMSteam` (`steam_bridge.gd`) mirrors six of them to Steam through the optional GodotSteam GDExtension. |
| `game/ui/` | Screens and widgets built in code on a fixed 1920×1080 `stage` centered for any aspect: `BMGameScreen`, `BMShopScreen` (with the Workshop picker), `BMTitleScreen`, `BMSplash` (launch splash; `-- --no-splash` skips it, `tools/shoot.py` passes that unless `BM_SPLASH=1`), `BMBoardView`, `BMTraySlot`, `BMBagView`, `BMPieceTile`, `BMCard` (Joker/item racks, shop offer cards, emblems), `BMHud` (marquee, tube, lamps, rolling counter, receipt), `BMWallet` (the Credits counter: coins fly in and count up, fly out on spending, click to flip the coin; presentation only), `BMBrickThrow` (the physics Emergency Brick), `BMTrophyCase` (achievement pages + records), `BMBadge` (medal widget), `BMAchievementToasts` (unlock plates), `BMStyle` (palette, fonts, 9-slice boxes, buttons, theme), `BMUI` (formatting, `fmt_score` for M/B/T/Q), `BMSettingsMenu` (tabbed Pause/Options, RUN page), `BMBossIntro` (boss cinematic), `BMTipPlate` (one tip), `BMTutorial` (POPS: corner helper, speech bubble, gibberish voice, pointer and spotlight), `BMRefreshLever` (the Refresh button drawn as an arcade lever). |
| `game/presentation/` | Visual-only: `BMCardArt` (card portraits and badge sheets from `tools/art/gen_cards.py`), `BMFinishes` (finish catalog: names, animation phase, glow, particles), `BMBlockPainter` (plastic blocks, animated finish faces, glow pass, stamp badges), `BMSwirlBackground` + `shaders/bg_swirl.gdshader`, `BMCrtLayer` + `shaders/crt.gdshader` (also remaps mouse input through the warp), `BMFx` (capped particle layer, pop text, shake), `BMMoodLayer` + `shaders/mood.gdshader` (boss hazard frame, danger vignette, heat haze, flashes; edges only, sized in stage pixels), `BMCursor` (custom hardware cursors + click effects, under the CRT layer). |
...
| `game/i18n/`, `locale/` | Translations (see [docs/localization.md](docs/localization.md)): `BMLoc` (languages, `t`/`tn` for UI text, `m`/`mn` markers for rules text, `tf` to show rules text, number and list separators) and the generated `BMMessages.PATTERNS`; `locale/messages.pot` plus one gettext `.po` per language (es, fr, it, de, nl, pl, pt_BR, ja, zh_CN, zh_TW), loaded at runtime. |
| `game/main.gd` + `main.tscn` | App root `BMMain`: routes screens, the single `act()` entry point, autosave, pause, input-map registration. |
| `tests/e2e/` | **The main test suite.** `run_e2e.gd` boots `main.tscn` per scenario with a sandboxed profile (one per checkout, so worktrees can test at the same time); `scenario_*.gd` (extend `BME2ECase`) drive the real app through its UI entry points and write artifacts to `build/e2e/`. |
...
| `tools/i18n/` | `extract.py` (template + `messages.gd` from `BMLoc` calls and `# i18n` catalog markers), `update_po.py` (merge into every `.po`), `check.py` (completeness, placeholders, edge spaces, capitals, glyph coverage), `po.py`. `tools/art/gen_cjk_fonts.py` subsets the Fusion Pixel CJK fallbacks. |
| `tools/` | `shoot.py` (focus-safe screenshot runner, see below), `BMAutoplayer` (preview-guided bot with a configurable shop policy), `simulate.gd` (quick balance probe), `experiments.gd` (paired-seed content experiments: curve / jokers / upgrades), `playtest.gd` + `playtest_report.py` (persona playtests: random / newcomer / steady / planner skill ladder, build archetypes, Kits, ceiling probes; report in `docs/playtests/`), `study.gd` + `study_report.py` (release study: a population of synthetic participants with item, shop, round-card and duplicate habits plus paired-seed experiment arms; logs every offer, purchase, item and round; `overtime_report.py` for runs continued into Overtime). Dev-only. |
...
- Kits: each Kit's rules and signature perk are data in `BMRunConfig.KITS` (`thrift_credits`, `interest_bonus`, `big_piece_chips`, `hand_credits`) read by `BMRun._win_round`, `BMRun._apply_hand` and `BMResolver`; starter bags in `BMPieces.STARTER_BAGS`. A locked Kit's card shows only its name, requirement and progress.
- Practice seeds: a run on a player-chosen seed has `BMRun.custom_seed` (fixed at `new_run` via `BMMain.start_new_run(..., custom_seed)`); `BMMain.act` then skips achievements, lifetime notes, records and `BMSaveStore.record_run`. New progression hooks must respect it.
- Veteran Chips live on the bag piece (`piece.veteran`); anything that rebuilds a piece (`BMBag.add_piece`, `BMPieces.to_dict/from_dict`) must carry it.
- Steam achievements: `BMSteam.MAP` maps local achievement ids to Steamworks API names; keep it in sync with `ACH` in `tools/store/build_achievements.py` and never rename a published API name. `BMSteam` is a no-op without the GodotSteam singleton and never reports from a sandboxed profile or a `--script` tool.
...
- Translations (owner request, 2026-09-25; [docs/localization.md](docs/localization.md)): every string a player reads goes through `BMLoc.t`/`tn` (UI) or is a catalog field listed in the const's `# i18n` marker (read through the catalog's display helper). Rules code writes English with `BMLoc.m`/`mn` so saves, records and results stay language-free; UI shows those strings with `BMLoc.tf`. Never compare against displayed text. Keep placeholder order (GDScript `%` cannot reorder; use `{named}` with `format` where a language must). Numbers through `BMUI.fmt_int`. After changing text: `python tools/i18n/extract.py && python tools/i18n/update_po.py`, translate, `python tools/art/gen_cjk_fonts.py` if CJK text changed, `python tools/i18n/check.py`. The SYSTEM setting follows Steam's game language (`BMSteam.game_language()` mapped by `BMLoc.STEAM_CODES`), so `BMSteam.start()` runs before `BMLoc.apply`; the Steamworks language lists must name exactly `BMLoc.LANGUAGES` ([store/steam/LANGUAGES.md](store/steam/LANGUAGES.md)). `-- --lang-report` makes a build print the translations and CJK fonts it carries and quit; `tools/steam/build_steam.sh` fails without all of them. Store text and images for every language: `tools/store/build_store.py` (`text` for the images with words), achievements: `tools/store/build_achievements.py` (texts from the `.po` files).
- Text must fit in every language: a fixed rect gets its size from the measured text once the control is in the tree, or steps its font down; one line drawn in code goes through `BMUI.draw_fit`/`fit_size` (grid sizes only). `tests/e2e/scenario_languages.gd` walks every screen in all eleven languages and fails on missing glyphs, text escaping its panel or the window, controls grown past their rect, cut lines without a tooltip, words split mid-line, untranslated text and drawn text or fitted buttons that do not fit; run it after any UI or text change.
...
# Translations: extract, merge, check (then the languages E2E scenario)
python tools/i18n/extract.py && python tools/i18n/update_po.py && python tools/i18n/check.py
...
# Release study: population (participants, first id, output, runs each) or an experiment arm (see ARMS)
"<godot>" --headless --path . --script res://tools/study.gd -- population 30 1 user://pop_0.json 3
"<godot>" --headless --path . --script res://tools/study.gd -- items_never 120 7001 user://items_never.json
python tools/study_report.py out_dir pop_0.json items_never.json
# ... with Overtime: add "overtime" after runs_each for the population, or use an ot_* arm; then
python tools/overtime_report.py out_dir pop_ot_0.json ot_best.json
...
python tools/art/gen_lever.py [contact.png]
python tools/art/gen_cursor.py
