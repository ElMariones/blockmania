...
- Kits: each Kit's rules and signature perk are data in `BMRunConfig.KITS` (`thrift_credits`, `interest_bonus`, `refill_per_line`, `hand_credits`) read by `BMRun._win_round`, `BMRun._apply_hand` and `BMResolver`; starter bags in `BMPieces.STARTER_BAGS`. A locked Kit's card shows only its name, requirement and progress.
